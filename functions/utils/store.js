...
export async function writeRoot(env, data) {
  ...
  try {
    ...
    // one more full retry after short wait
    try {
      await sleep(500);
      await jsonbinWrite(env, payload);
      return { ok: true, backend: 'jsonbin' };
...
/** Safer update: re-read before write to reduce lost updates */
export async function mutateIssues(env, mutator) {
  let lastErr = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    const { data, error } = await readRoot(env);
    if (error && (!data.issues || data.issues.length === 0) && attempt < 2) {
      await sleep(300);
      continue;
    }
    ...
    if (saved.ok) {
      return { ok: true, issues, backend: saved.backend, ...result };
    }
    lastErr = saved.error;
    await sleep(350 + attempt * 300);
  }
  return { ok: false, error: lastErr || 'Could not save. Try again.' };
}
