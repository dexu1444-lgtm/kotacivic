...
/** Discord embed logger
 * sendDiscord(env, { title, description, fields, color })
 * or sendDiscord(env, "plain string") for simple text embed
 */
export async function sendDiscord(env, payload) {
  ...
  if (typeof payload === 'string') {
    embed = {
      title: 'Civic AI Log',
      description: String(payload).slice(0, 2000),
      color: 0x3b82f6,
      footer: { text: 'Civic AI · ' + now },
      timestamp: new Date().toISOString()
    };
  } else {
    const p = payload || {};
    embed = {
      title: (p.title || 'Civic AI Log').slice(0, 256),
      description: p.description ? String(p.description).slice(0, 2000) : undefined,
      color: typeof p.color === 'number' ? p.color : 0x3b82f6,
      fields: Array.isArray(p.fields)
        ...
        : undefined,
      footer: { text: (p.footer || 'Civic AI') + ' · ' + now },
      timestamp: new Date().toISOString()
    };
  }

  const body = { embeds: [embed], username: 'Civic AI' };
  try {
    if (webhook) {
      await fetch(webhook, {
        method: 'POST',
