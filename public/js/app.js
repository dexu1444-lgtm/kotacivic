...
async function api(path, options = {}, retries = 2) {
  let last = { ok: false, message: 'Network error' };
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      try { data = text ? JSON.parse(text) : {}; } catch (_) {
        data = { message: 'Bad response (' + res.status + '): ' + text.slice(0, 120) };
      }
      last = { ok: res.ok, status: res.status, ...data };
      if (!last.message && !res.ok) last.message = 'HTTP ' + res.status;
      ...
      last = { ok: false, message: 'Network error — check connection' };
      if (attempt < retries) {
        await new Promise((r) => setTimeout(r, 500 + attempt * 500));
        continue;
      }
...
function animateValue(el, end, duration) {
  ...
  function frame(now) {
    ...
    el.textContent = Math.round(endNum * eased);
...
async function loadStats() {
  ...
  animateValue(document.getElementById('statTotal'), stats.total, 800);
  animateValue(document.getElementById('statResolved'), stats.resolved, 800);
  animateValue(document.getElementById('statOpen'), stats.open, 800);
  animateValue(document.getElementById('statRejected'), stats.rejected, 800);
  return stats;
}
